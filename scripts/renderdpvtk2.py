...
def render_dpvtk(filename, output_png):
    ...
    print("rank " + str(rank) + " about to callpydpvz.DPvzVtk()")
    archive = pydpvz.DPvzVtk(filename, pydpvz.DPvzMode.DPvzReadOnly, comm, False)
    print("rank " + str(rank) + " returned fromcallpydpvz.DPvzVtk()")
    ...
    print("rank " + str(rank) + " about to SaveScreenshot")
    if rank == 0:
        ...
        paraview_simple.SaveScreenshot(output_png, view, ImageResolution=[1920, 1080])
    print("rank " + str(rank) + " returned from SaveScreenshot")
