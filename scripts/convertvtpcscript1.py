...
import pydpvz
from pydpvz.vtk_serializer import serialize_partitioned_dataset_collection

def main():
    parser = argparse.ArgumentParser(description="Convert VTPC/Exodus II to DPVTK")
    ...
    for cycle, t in enumerate(timesteps):
        ...
        # Fetch local rank data (must be run in symmetric mode to get distributed pieces)
        local_data = reader.GetClientSideObject().GetOutputDataObject(0)
        
        # Serialize to memory
        xml_string = serialize_partitioned_dataset_collection(local_data, rank, cycle, t)
